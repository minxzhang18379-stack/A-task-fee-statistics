...
*   **🔒 工业级加盐哈希与 JWT 动态鉴权**：
    ...
    *   **可视化“密码管理终端”与权限熔断**：
        ...
        *   **普通成员 (Member)**：极简只读/防误触安全隔离。登录后不仅限制写操作，前端还会在 DOM 中物理移除“新增任务”按钮、“导入”按钮、首列“勾选多选框”、整列“编辑/删除”操作按钮，以及设置中的密码管理终端。同时，云端服务端 API 写入接口（POST/PUT/DELETE）会对 member 角色执行严格的 admin 鉴权阻断（返回 403 Forbidden），确保数据大盘防篡改、防误触。
*   **📊 强大的数据导入导出引擎**：
...
*   `src-tauri/target/release/bundle/nsis/PANN Task Manager_1.1.1_x64-setup.exe` (自动复制并同步至您的项目根目录下)
